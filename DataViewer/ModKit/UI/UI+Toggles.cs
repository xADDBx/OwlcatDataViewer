    ...
    public static partial class UI {
        ...
        private static bool TogglePrivate(
            ...
            if (width == 0 && !disclosureStyle) {
                width = toggleStyle.CalcSize(new GUIContent(title.bold())).x + GUI.skin.box.CalcSize(UI.CheckOn).x + 10;
            }
            options = options.AddItem(width == 0 ? AutoWidth() : Width(width)).ToArray();
            if (!disclosureStyle) {
                title = value ? title.bold() : title.color(RGBA.medgrey).bold();
                if (UI.CheckBox(title, value, isEmpty, toggleStyle, options)) { value = !value; changed = true; }
            } else {
                if (UI.DisclosureToggle(title, value, isEmpty, options)) { value = !value; changed = true; }
            }
            return changed;
        }
        ...
        public static bool Toggle(string? title, ref bool value, string? on, string? off, float width = 0, GUIStyle? stateStyle = null, GUIStyle? labelStyle = null, params GUILayoutOption[] options) {
            ...
            options = options.AddItem(width == 0 ? AutoWidth() : Width(width)).ToArray();
            title = value ? title.bold() : title.color(RGBA.medgrey).bold();
            if (UI.Toggle(title, value, on, off, stateStyle, labelStyle, options)) { value = !value; changed = true; }
            return changed;
        }
        public static bool Toggle(string? title, ref bool value, params GUILayoutOption[] options) {
            options = options.AddDefaults();
            var changed = false;
            if (UI.CheckBox(title, value, false, toggleStyle, options)) { value = !value; changed = true; }
            return changed;
        }
