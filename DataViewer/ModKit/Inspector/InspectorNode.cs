...
public class InspectorNode : IComparable {
    ...
    public string TypeNameText {
        get {
            field ??= Helper.GetNameWithGenericsResolved(m_FieldType);
            return field;
        }
    }
    ...
    public string LabelText {
        get {
            if (field == null) {
                ...
                string typeName;
                if (Helper.PrimitiveTypes.Contains(ConcreteType)) {
        ...
        get {
            if (field == null) {
                if (ConcreteType != m_FieldType) {
                    field = Helper.GetNameWithGenericsResolved(ConcreteType).Yellow();
                } else {
                    field = "";
                }
            }
    ...
    public InspectorNode(string name, string path, Type type, object? value, InspectorNode? parent, string containerPrefix, bool isStatic = false, bool isPublic = true, bool isPrivate = false, bool isCompilerGenerated = false) {
        ...
        Path = path + name + "/";
#if KM
        if (value is BlueprintReference bpRefBase)
            bpRefBase.Get();
#else
        ...
        if (Helper.IsNullableT(type, out var maybeUnderlying)) {
